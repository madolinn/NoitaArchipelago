...
class NoitaWorld(World):
    ...
    def generate_early(self) -> None:
        if not self.multiworld.get_player_name(self.player).isascii():
            raise Exception("Noita-Killsanity yaml's slot name has invalid character(s).")
    ...
    def fill_slot_data(self) -> Dict[str, Any]:
        return self.options.as_dict("death_link", "victory_condition", "path_option", "hidden_chests",
                                    "pedestal_checks", "orbs_as_checks", "bosses_as_checks", "animals_as_checks", "animals_need_credit", "extra_orbs", "shop_price")
